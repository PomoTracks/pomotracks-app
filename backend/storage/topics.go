...
func CreateTopic(topic Topic) (*mongo.InsertOneResult, error) {
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()

	collection := client.Database("pomotracks").Collection("topics")
	return collection.InsertOne(ctx, topic)
}

// ListTopics retrieves all topics from the database
